...
export default function CompsPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    min_sf: '', max_sf: '', min_price: '', max_price: '',
    min_date: '', max_date: '', sewer: ''
  })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-gray-900">Sale Comps</h1>
        <span className="text-sm text-gray-400">Long Island Industrial</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              <label className="text-xs text-gray-500 font-medium uppercase tracking-wide">Min Sale Price</label>
            ...
            <div>
              <label className="text-xs text-gray-500 font-medium uppercase tracking-wide">Max Sale Price</label>
            ...
            <div>
              <label className="text-xs text-gray-500 font-medium uppercase tracking-wide">Sale Date From</label>
              <input type="date" className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={filters.min_date} onChange={e => set('min_date', e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-gray-500 font-medium uppercase tracking-wide">Sale Date To</label>
              <input type="date" className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={filters.max_date} onChange={e => set('max_date', e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-gray-500 font-medium uppercase tracking-wide">Sewer</label>
              <select className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={filters.sewer} onChange={e => set('sewer', e.target.value)}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { setFilters({ county:'',city:'',address:'',zoning:'',min_sf:'',max_sf:'',min_price:'',max_price:'',min_date:'',max_date:'',sewer:'' }); setResults([]); setSearched(false) }} className="text-gray-500 px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-sm">
              Clear
            </button>
            {selected.size > 0 && (
              <a href={`/opv/new?comps=${[...selected].join(',')}`} className="ml-auto bg-green-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-green-700 transition-colors">
        ...
        {searched && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-6 py-3 border-b border-gray-100 flex items-center justify-between">
              <span className="text-sm text-gray-600 font-medium">{results.length} results {results.length === 200 ? '(showing first 200)' : ''}</span>
              {results.length > 0 && (
                <button onClick={() => setSelected(new Set(results.map(r => r.id)))} className="text-sm text-blue-600 hover:underline">
                  Select all
                </button>
              )}
            </div>
            {results.length === 0 ? (
              <div className="p-12 text-center text-gray-400">No results found. Try broadening your filters.</div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">Sale Price</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">$/SF</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Sale Date</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {results.map(r => (
                      <tr key={r.id} className={`hover:bg-gray-50 ${selected.has(r.id) ? 'bg-blue-50' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-800">{fmt(r.sale_price)}</td>
                        <td className="px-4 py-3 text-right text-gray-600">{r.price_per_sf ? `$${Number(r.price_per_sf).toFixed(2)}` : '—'}</td>
                        <td className="px-4 py-3 text-gray-600">{r.sale_date ? r.sale_date.substring(0,10) : '—'}</td>
