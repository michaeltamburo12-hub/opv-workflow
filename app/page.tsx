                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:8}} onClick={e=>e.stopPropagation()}>
                            {(item as Comp & {photo_url?:string}).photo_url
                              ? <img src={(item as Comp & {photo_url?:string}).photo_url} alt={item.address} style={{width:120,height:68,objectFit:'cover',borderRadius:5,border:`1px solid ${D.border}`,flexShrink:0}}/>
                              : item.address
                                ? <img src={`/api/street-view?address=${encodeURIComponent(item.address + ((item as Comp).city ? ', ' + (item as Comp).city + ', NY' : ', NY'))}`} alt={item.address} style={{width:120,height:68,objectFit:'cover',borderRadius:5,border:`1px solid ${D.border}`,flexShrink:0}} onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>
                                : <div style={{width:120,height:68,background:D.surface2,borderRadius:5,border:`1px dashed ${D.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:D.textMuted,flexShrink:0}}>No Photo</div>
                            }
