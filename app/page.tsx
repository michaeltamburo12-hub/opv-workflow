...
const DB_FIELD_OPTIONS=[
  ...
  // Common
  {value:'address',label:'Address'},{value:'city',label:'City'},{value:'town',label:'Town'},{value:'county',label:'County'},{value:'state',label:'State'},{value:'zip_code',label:'Zip Code'},{value:'property_type',label:'Property Type'},{value:'building_sf',label:'Building SF'},{value:'lot_size_ac',label:'Lot Size (acres)'},{value:'ceiling_height',label:'Ceiling Height'},{value:'loading_docks',label:'Loading Docks'},{value:'drive_ins',label:'Drive-In Doors'},{value:'power',label:'Power'},{value:'heat',label:'Heat'},{value:'parking',label:'Parking'},{value:'sprinkler',label:'Sprinkler'},{value:'sewer',label:'Sewer'},{value:'zoning',label:'Zoning'},{value:'real_estate_taxes',label:'RE Taxes ($/yr)'},{value:'notes',label:'Notes'},{value:'status',label:'Status'},{value:'tenant',label:'Tenant'},{value:'landlord',label:'Landlord'},
  // Sale comp fields
  {value:'sale_price',label:'Sale Price ($)'},{value:'asking_price',label:'Asking Price ($)'},{value:'price_per_sf',label:'Price Per SF'},{value:'sale_date',label:'Sale Date'},{value:'sale_type',label:'Sale Type'},{value:'buyer',label:'Buyer'},{value:'seller',label:'Seller'},{value:'listing_broker',label:'Listing Broker'},{value:'market',label:'Market'},{value:'submarket',label:'Submarket'},{value:'pricing_guidance',label:'Pricing Guidance'},{value:'availability_type',label:'Availability Type'},{value:'loopnet_url',label:'LoopNet URL'},
  // Lease comp fields
  {value:'transaction_date',label:'Transaction Date'},{value:'asking_rent',label:'Asking Rent ($/SF)'},{value:'deal_rent',label:'Deal Rent ($/SF)'},{value:'rent_type',label:'Rent Type'},{value:'taxes',label:'Taxes'},{value:'lease_term_years',label:'Lease Term (Years)'},{value:'rent_concession_months',label:'Rent Concession (Months)'},{value:'ti_ll_work',label:'TI / LL Work'},{value:'mgmt_fee_pct',label:'Mgmt Fee (%)'},
]
...
function DatabaseManager() {
  ...
  const startEdit = (row: BrowseRow) => {
    ...
    else if (tab==='lease-comps') setLeaseCompForm({transaction_date:str((row as Record<string,unknown>).transaction_date),address:str(row.address),town:str((row as Record<string,unknown>).town),county:str(row.county)||'Nassau',property_type:str((row as Record<string,unknown>).property_type)||'Industrial',building_sf:str(row.building_sf),lot_size_ac:str((row as Record<string,unknown>).lot_size_ac),ceiling_height:str((row as Record<string,unknown>).ceiling_height),loading_docks:str((row as Record<string,unknown>).loading_docks),drive_ins:str((row as Record<string,unknown>).drive_ins),office_sf:str((row as Record<string,unknown>).office_sf),power:str((row as Record<string,unknown>).power),heat:str((row as Record<string,unknown>).heat),sprinkler:str((row as Record<string,unknown>).sprinkler),parking:str((row as Record<string,unknown>).parking),sewer:str((row as Record<string,unknown>).sewer),zoning:str((row as Record<string,unknown>).zoning),re_taxes:str((row as Record<string,unknown>).re_taxes),asking_rent:str((row as Record<string,unknown>).asking_rent),deal_rent:str((row as Record<string,unknown>).deal_rent),rent_type:str((row as Record<string,unknown>).rent_type)||'NNN',taxes:str((row as Record<string,unknown>).taxes),lease_term_years:str((row as Record<string,unknown>).lease_term_years),rent_concession_months:str((row as Record<string,unknown>).rent_concession_months),ti_ll_work:str((row as Record<string,unknown>).ti_ll_work),mgmt_fee_pct:str((row as Record<string,unknown>).mgmt_fee_pct),escalations:str((row as Record<string,unknown>).escalations),tenant:str((row as Record<string,unknown>).tenant),landlord:str((row as Record<string,unknown>).landlord),status:str(row.status)||'Active',notes:str((row as Record<string,unknown>).notes)})
    else if (tab==='lease-avails') setLeaseAvailForm({address:str(row.address),town:str((row as Record<string,unknown>).town),county:str(row.county)||'Nassau',property_type:str((row as Record<string,unknown>).property_type)||'Industrial',building_sf:str(row.building_sf),lot_size_ac:str((row as Record<string,unknown>).lot_size_ac),office_sf:str((row as Record<string,unknown>).office_sf),ceiling_height:str((row as Record<string,unknown>).ceiling_height),loading_docks:str((row as Record<string,unknown>).loading_docks),drive_ins:str((row as Record<string,unknown>).drive_ins),power:str((row as Record<string,unknown>).power),heat:str((row as Record<string,unknown>).heat),sprinkler:str((row as Record<string,unknown>).sprinkler),parking:str((row as Record<string,unknown>).parking),sewer:str((row as Record<string,unknown>).sewer),zoning:str((row as Record<string,unknown>).zoning),re_taxes:str((row as Record<string,unknown>).re_taxes),asking_rent:str((row as Record<string,unknown>).asking_rent),rent_type:str((row as Record<string,unknown>).rent_type)||'NNN',taxes:str((row as Record<string,unknown>).taxes),lease_term_years:str((row as Record<string,unknown>).lease_term_years),escalations:str((row as Record<string,unknown>).escalations),landlord:str((row as Record<string,unknown>).landlord),listing_broker:str((row as Record<string,unknown>).listing_broker),loopnet_url:str((row as Record<string,unknown>).loopnet_url),status:str(row.status)||'Available',notes:str((row as Record<string,unknown>).notes)})
      ...
      {subTab==='add' && tab==='lease-avails' && (
        <Card>
          ...
          <div style={G3}>
            ...
            <Field label="Office SF (If applicable)"><Input type="number" value={leaseAvailForm.office_sf} onChange={e=>setLA('office_sf',e.target.value)}/></Field>
...
function OPVReport({subject,comps,leaseComps,leaseAvails=[],avails,analytics,aiText,setPage,frozenHTML,setFrozenHTML,photoUrls={},opvId}: {subject:SubjectForm|null,comps:Comp[],leaseComps:LeaseComp[],leaseAvails?:LeaseComp[],avails:Avail[],analytics:AnalyticsData|null,aiText:string,setPage:(p:string)=>void,frozenHTML:string|null,setFrozenHTML:React.Dispatch<React.SetStateAction<string|null>>,photoUrls?:Record<string,string>,opvId?:string|null}) {
  ...
  const clearPhoto = (key: string) => {
    setEditingKey(null)
    // Compute the street-view URL for this slot
    let svUrl = ''
    if (key === 'subject_cover' && subject) {
      svUrl = `/api/street-view?address=${encodeURIComponent(subject.address+(subject.city?', '+subject.city:'')+', NY')}`
    } else if (key.startsWith('comp_')) {
      const c = comps.find(c=>String(c.id)===key.replace('comp_',''))
      if (c?.address) svUrl = `/api/street-view?address=${encodeURIComponent(c.address+(c.city?', '+c.city:'')+', NY')}`
    } else if (key.startsWith('avail_')) {
      const a = avails.find(a=>String(a.id)===key.replace('avail_',''))
      if (a?.address) svUrl = `/api/street-view?address=${encodeURIComponent(a.address+(a.city?', '+a.city:'')+', NY')}`
    }
    // Set override TO the street-view URL (not delete it).
    // This way the DOM sync picks photoOverrides[key] = svUrl over any photoUrls value,
    // which would otherwise keep re-applying the custom photo.
    if (svUrl) {
      setPhotoOverrides(p=>({...p,[key]:svUrl}))
    } else {
        ...
        <SubHead>ABOUT THE BROKERS</SubHead>
          ...
          <div style={{display:'flex',gap:24,alignItems:'flex-start',marginBottom:36,paddingBottom:36,borderBottom:'1px solid #eee'}}>
            ...
            <div style={{flex:1}}>
              ...
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Jason Miller is Co-Founder and Managing Principal at Premier Commercial Real Estate with over 17 years experience, and is widely recognized as one of Long Island's top commercial real estate professionals. During his career, Mr. Miller has represented a substantial number of satisfied clients and customers, successfully completing an extraordinary number of sales and leasing transactions, totaling several million square feet of space.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Mr. Miller began his real estate career as a Sales Associate at Sutton &amp; Edwards Inc (now Colliers International, LI Inc.) and was quickly promoted to the position of Senior Director, Industrial Properties for Long Island. However, after a number of successful years there, Mr. Miller felt the structure and rigid policies of the larger national real estate brokerage firm, limited his ability to provide the &ldquo;timely and responsive&rdquo; flexibility that his customers and clients demanded. So, in October of 2013, along with his partner Jeff Schwartzberg, he created Premier Commercial Real Estate, a brokerage firm strictly focused on delivering ultimate customer service. Premiers&apos; &ldquo;easy to do business with&rdquo; approach, combined with our in-depth and extensive market knowledge, is truly a winning formula/platform that our clients have quickly embraced.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Since Premiers&apos; inception, Jason and his company have received a number of the industry&apos;s top awards. Some examples include: In 2015 Premier was recognized by LIBN as &ldquo;commercial brokerage company of the year&rdquo;; Premier was awarded top honors in 2015 for completing the largest industrial sale transactions in both Nassau County and Suffolk County; In 2016 Premier received yet another award for completing Nassau County&apos;s largest industrial sales transaction; In 2016 Jason was chosen from hundreds of candidates to receive LIBN&apos;s prestigious 40 under 40 award; and each year over the past decade, Jason has received the Co-Stars&apos; Power Broker Award.</p>
              <p style={{fontSize:11.5,lineHeight:1.85}}>Today, Mr. Miller is widely recognized as a dedicated, skilled, and highly knowledgeable real estate professional, which has resulted in him being chosen to represent some of Long Island&apos;s largest Landlords, properties and portfolios. He has a proven track-record representing diverse industrial properties that include Manufacturing Facilities, Distribution/Fulfillment Centers, Cold Storage, Flex/R&amp;D Buildings, and more.</p>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:24,alignItems:'flex-start',marginBottom:36,paddingBottom:36,borderBottom:'1px solid #eee'}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontWeight:600,fontSize:12,color:'#555',fontStyle:'italic',marginBottom:12}}>Managing Principal</div>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>JB (Jeff) Schwartzberg, Co-Founder and Managing Principal at Premier Commercial Real Estate, has developed a vast background and proven track record in commercial real estate spanning over three decades. Mr. Schwartzberg brings that knowledge to Premier, leading its&apos; day-to-day business operations. He is widely recognized in the industry as one of Long Island&apos;s Top commercial real estate professionals.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Before becoming a full time Broker, Jeff enjoyed a successful and extensive 20-year career, holding several key &ldquo;senior executive&rdquo; positions in the Defense Industry, with Sperry/Unisys/Loral/Lockheed Martin Corporations. This was followed by several years as a Senior Investment Officer at First Industrial Realty Trust, a Chicago based REIT, which acted as a natural springboard for his brokerage career.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Immediately prior to creating Premier in 2013, Mr. Schwartzberg spent more than a decade with Colliers International, ultimately rising to the position of Senior Executive Director, Industrial Properties-Long Island. While there, he was honored with a &ldquo;Person of the Year&rdquo; award. Further, he has been named a Co-Star &ldquo;Power Broker&rdquo; every year and has participated in industrial property sales and leases involving millions of square feet, representing owners, buyers and tenants. In 2023, Jeff was honored &amp; received Long Island Business News&apos; most prestigious award &ldquo;Top Commercial Broker on Long Island.&rdquo; Mr. Schwartzberg and his partner are often tapped by various commercial real estate industry leaders for their in-depth knowledge of the local market, their problem-solving skills and their views on future trends.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Mr. Schwartzberg was a long-time member of the Board of Directors of Long Islands&apos; Commercial Industrial Brokers Society (CIBS) and in fact, served eight years on its&apos; executive board, ultimately becoming its&apos; President, a position that he held for two years. Today he sits on the Advisory Committee to the board. He had also earned membership into the Society of Industrial and Office Realtors (SIOR), where he is a former member in good standing. One of Mr. Schwartzberg&apos;s additional noteworthy and proud accomplishments is his having earned status as a certified NYS instructor of continuing education for real estate professionals.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Community-minded, he served for 14 years as a Village Official in the Village of Roslyn Estates; initially serving as Chairman of the Board of Zoning Appeals (BZA) and ultimately, being elected for 3 consecutive terms (receiving 99 percent of votes) as Village Mayor. Jeff also served for 17 years on the Board of &ldquo;Big Brothers Big Sisters of Long Island&rdquo;, that included several terms as the Board Chairperson.</p>
              <p style={{fontSize:11.5,lineHeight:1.85}}>Mr. Schwartzberg earned an undergraduate degree in Accounting from Queens College and holds an MBA in Financial Management from Iona College. Today, Jeff lives on the water in Massapequa, where he enjoys spending time with his family and admiring the beauty of Long Island&apos;s Great South Bay. He is not only a &ldquo;Bronx Guy,&rdquo; but also a &ldquo;Corvette Guy.&rdquo; He is proud of his accomplishments and is always eager to share his knowledge and expertise with his clients and customers.</p>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:24,alignItems:'flex-start',marginBottom:32}}>
            ...
            <div style={{flex:1}}>
              ...
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Desmond Mullins is a seasoned commercial real estate broker, making waves in the Long Island market with an impressive track record of success. Born and raised in Hauppauge, NY, Desmond currently calls Long Beach home but operates throughout the various business parks across Nassau and Suffolk County. He specializes in landlord, tenant, seller, and buyer representation.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>In 2025, Desmond was promoted to Partner at Premier Commercial Real Estate, a milestone that reflects his leadership and continued impact on the firm&apos;s growth. That same year, he was recognized as one of LIBN&apos;s Emerging Leaders Under 30. His earlier accolades include LIBN&apos;s 30 Under 30 (2021), the CIBS Rising Star Award (2022), and leading his team to be named LIBN&apos;s Top Industrial Brokerage Team (2024).</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>Desmond&apos;s expertise spans transactions exceeding a total value of more than $300 million. He remains an active member of CIBS (Commercial Industrial Brokers Society) and HIA (Hauppauge Industrial Association), contributing to the growth and innovation of Long Island&apos;s industrial market.</p>
              <p style={{fontSize:11.5,lineHeight:1.85,marginBottom:8}}>A globetrotter by nature, Desmond travels across the United States, Europe, and South America to gain insights and global perspectives. He holds a BS in Business Administration with a concentration in entrepreneurship from the School of Management at Binghamton University.</p>
