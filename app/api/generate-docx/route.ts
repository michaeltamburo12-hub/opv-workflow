  ...
  try {
    ...
    if (isLease && leaseComps?.length > 0) {
      ...
      for (let i = 0; i < leaseComps.length; i++) {
        ...
        const ip = imgPara(await fetchImg(photoUrls[c.id] || ''))
        ...
        children.push(propTable([
          ...
          ['LOADING', c.loading_docks || c.drive_ins ? `${c.loading_docks || '—'} Docks / ${c.drive_ins || '—'} Drive-In` : '—', true],
    ...
    if (includeAvails && availList.length > 0) {
      ...
      if (isLease) {
        ...
        for (let i = 0; i < availList.length; i++) {
          ...
          const ip = imgPara(await fetchImg(photoUrls[a.id] || ''))
          if (ip) children.push(ip)
          children.push(propTable([
